...
# To use : bsub < ./bsub_gene_trees_woutgroup.sh
...
cd ../files/aa_ortholog_families_aligned_woutgroup/
# Move to the folder containing all the families one wants to compare
all_families=$( find family* )

if [ ! -d "../phylogenetic_trees/aa_gene_trees_woutgroup/all_families" ] ; then
   mkdir -p ../phylogenetic_trees/aa_gene_trees_woutgroup/all_families
fi
...
for family in $all_families; do
    ...
    raxmlHPC-HYBRID -m PROTGAMMAWAG -p 12345 -s ../../../aa_ortholog_families_aligned_woutgroup/$family \
