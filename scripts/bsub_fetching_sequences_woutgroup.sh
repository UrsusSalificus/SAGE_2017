...
# To use : bsub < ./bsub_fetching_sequences_woutgroup.sh
...
ortholog_file=$'../files/woutgroup_orthologs_only'
all_families=$( less $ortholog_file | wc -l )
all_families_range=$( seq 1 1 $all_families )

mkdir ../files/aa_ortholog_families_NOT_aligned_woutgroup/
...
python3 fetching_sequences.py .faa ../files/aa_ortholog_families_NOT_aligned_woutgroup/ $ortholog_file
